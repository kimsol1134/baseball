...
// Prepare iOS 1.2.8, attach a processed build, and submit for manual release.
// Usage: node tools/asc-submit-1-2-8.mjs [--dry-run] prepare|attach|inspect|submit [buildNumber]
//
// What's New is read from marketing/appstore/RELEASE_NOTES_1.2.8.md (ko / en / ja).
// Build number is a CLI argument (1.2.7 was 65). Do not hardcode it.
...
function reviewNotes() {
  ...
  return `Version 1.2.8${buildLabel} expands the fictional professional season with a choice every three weeks and a result card three weeks later. At spring camp a player can request a starter, middle-relief, or closer role. Unfamiliar baseball terms open a short definition, and each choice leads with a one-line effect summary. Four new decisions cover outing spacing, a farm-club reset, taking a new pitch into games, and a veteran's advice. Stability improvements are included.
...
function plannedCalls(action) {
  ...
  return [
    "prepare: create 1.2.8, copy localizations from 1.2.7, set whatsNew and review notes",
...
else throw new Error("Usage: node tools/asc-submit-1-2-8.mjs [--dry-run] prepare|attach|inspect|submit [buildNumber]");
