...
### 6.2 스카우팅 버그 수정과 남은 격차 (2026-07-26 2차)

§6.1의 "커널이 투수에게 유리하다"는 진단은 **틀렸다.** 투구 단위를 재 보니 실제 야구와
거의 일치했다(존 통과율 0.492 / 존 밖 스윙 0.278 / 볼 비율 0.367 / 헛스윙 0.120).
배치 검사 설정에서 피안타율도 .228로 정상이었다. 어긋난 것은 **자동 등판 시뮬레이터**뿐이었다.

**찾은 버그**: `AutoOutingSimulator`가 타자의 강점 코스와 약점 코스를 각각 독립적으로
무작위 추출해서 **11%의 타자가 강점과 약점이 같은 칸**이었다. 그러면 포수가 약점인 줄 알고
타자가 가장 잘 치는 코스를 요구한다. 실제 스카우팅에서는 있을 수 없다. 약점을 강점의
대칭점으로 잡도록 고쳤다.

| 지표 | 수정 전 | 수정 후 | 실제 야구 |
|---|---|---|---|
| 피안타/9 | 11.95 | **10.45** | 8.2 |
| 볼넷/9 | 1.83 | **2.39** | 3.2 |
| 실점/9 | 3.48 | **2.86** | 4.4 |

피안타와 볼넷은 실제 쪽으로 왔지만 **실점은 오히려 멀어졌다** — 안타가 줄었으니 당연하다.
밴드(2.6~4.6) 하단에 붙었다.

**남은 원인은 장타 부족이다.** 홈런/9 0.69(실제 1.2), 홈런/피안타 0.066(실제 0.14).
안타가 대부분 단타라 출루 1명당 실점이 0.226에 그친다(실제 0.386). 주자는 쌓이는데
홈에 들어오지 못한다.

**다음 작업**: 커널의 타구 결과 분포에서 장타 비중을 올린다. 리그 평균 타자 기준 홈런/타석은
0.012로 밴드(0.005~0.032) 하단이라 올릴 여지가 있다. **위험**: 파워 단조성 검사가 파워 60에서
홈런/타석 0.033을 재고 밴드가 0.02~0.07이라, 기준선을 올리면 상한을 넘길 수 있다.
장타 비중과 파워 민감도를 함께 봐야 한다. 이건 커널 변경이므로 별도 작업으로 둔다.
