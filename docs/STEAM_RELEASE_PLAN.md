# Project Diamond Soul Steam 출시 계획
...
1. **Project Diamond Soul 정식판**: 고교 커리어부터 프로 은퇴까지 포함하는 유료 게임.
2. **Project Diamond Soul Demo**: 핵심 투구, 성장, 관계, 첫 중요 경기까지 보여 주는 무료 데모.
...
현재 코드는 `steam_full`, `steam_demo`, `web_teaser` 빌드를 구분한다. 정식판만 프로 커리어를 포함하며, 설정이 빠진 프로덕션 빌드는 데모로 닫힌다.
...
| Windows Steam판 | 미준비 | Windows 데포 산출물과 실기기 Steam 실행을 아직 검증하지 않음 |
| macOS Steam판 | 미준비 | 기능 번들은 있으나 Developer ID 서명과 Apple 공증이 없음 |
| Steam 데모 | 개발 시작 가능 | 정식판/데모 빌드 권한은 분리했으나 데모 종료 지점과 데포가 없음 |
| 모바일 웹 | 보조 채널 | 반응형·웹 실행 경계의 기반만 유지하고 본편보다 우선하지 않음 |

따라서 지금 앱을 Steam에 바로 제출할 수는 없다. 다음 구현 순서는 **Steam용 파일 저장 → Windows 무설치 데포 → 깨끗한 Windows QA → 데모 범위 고정 → Steamworks 등록·스토어 검토**다.
