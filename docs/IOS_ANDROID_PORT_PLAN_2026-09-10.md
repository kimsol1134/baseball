...
| Phase 4 투구 손맛 | **곡선·햅틱·흔들림·오디오·퍼펙트 타이밍 완료 / 장식 미착수** | `2fa19360`, `2d3219f9` + 퍼펙트 비행 15% 단축·삼진 콜 0.2초 선행(`PitchFeedbackTimeline`, 검사 4개). 잔상·실밥 2줄·판정 도장은 미착수 — 순수 장식이라 시뮬레이터 육안 확인이 게이트다 |
...
| Phase 8 문구·검증 | 부분 | 이식한 규칙의 코어 테스트를 추가했고, 자격 보드·시즌 비교·계보 비교의 ko/en/ja 45개를 넣어 `check:ios-localization` 통과. 실존 이름 검색과 UI 테스트·스모크는 미착수 |
