...
| Phase 2 프로 규칙 10→13 | **v12·v13 커널 이식 완료 / 버전은 아직 10** | `e62f2618`(자리) + 이후 커밋들. 자책점 원장·수비 실책·승계 주자 정산·투구 수 피로·교체 규칙까지 올렸고 등판 단위 밸런스는 안드로이드 근처에 들어왔다. 커리어 단위 밴드가 아직 안 맞아 `currentRulesVersion`은 10 그대로다. 아래 §2.1·§2.2 |
...
| Phase 6 대화·진로 UX | 미착수 | |
| Phase 7 실패 분류·복구·영수증 | 미착수 | |
| Phase 8 문구·검증 | 부분 | 이식한 규칙의 코어 테스트는 추가했고, ko/en/ja 신규 문구는 없음 |
...
진행: 2-A·2-B·2-C·2-D 완료(버전 게이트 뒤), 2-F 승격분 완료, 2-E와 2-F 명예의 전당분 미착수. §2.1의 표를 참조.
...
- 2-F **승격·명예의 전당.** 승격은 **완료** — `ProCallUpRules`를 이름 그대로 이식하고 `planWeek`의 인라인 조건을 그것으로 바꿨다. 화면은 `ProAdvancementRules`를 통해 같은 함수를 읽으므로 커널과 조건이 갈라질 수 없다(성장 곡선 계획 G-4b). 명예의 전당 재조정은 미착수.
