...
| Phase 6 대화·진로 UX | **6-A·6-B·6-D 완료 / 6-C·6-E 미착수** | `263b49bf`·`15331b71`. 화자 배정과 커널 미리보기를 이식하고(`ProConversationPresentation`, `BaseballIOSDomain`), 프로·고교 대화가 `ConversationStage`+`ConversationChoiceCard` 한 관용구를 공유한다. 84pt 초상·풀카드 셋·모달 없음·결과 유지. 칩은 선언된 `ProDecisionEffect`가 아니라 **같은 시드로 커널을 돌린 결과**다 — clamp된 능력, 다듬는 구종, 3주 약속이 이제 선택 전에 보인다. 6-D(`6eaf106d`)는 계약 확인 알럿을 같은 화면의 확인 블록으로 바꿨다 — 연봉·계약금·보장 총액 라벨은 이미 갈라져 있었고, 선택은 이전부터 명령 0회였다 |
