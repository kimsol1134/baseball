...
이 머신은 Unity Editor entitlement가 없어 import 전에 종료되며 서명 AAB와 실제 스마트폰도
없다. 따라서 아래 어느 행도 `UNITY`, `DEVICE`, `ACCEPTED`로 올리지 않는다. `SOURCE`는 출시
승인이 아니라, 라이선스·빌드·기기 게이트에 넘길 수 있는 구현 범위를 나타낸다.
...
| P-002 | 선수 생성 | HighSchoolSetupView | SOURCE | `Application/HighSchool/HighSchoolSetupCatalog.cs`; `HighSchoolSetupContractTests`, `StoreRuntimeProjectionTests` | 대기 |
...
| P-005 | 학교 선택 | HighSchoolCareerView | SOURCE | 19지역·4후보 catalog와 실제 choice payload; `HighSchoolSetupContractTests` | 대기 |
| P-006 | 훈련 | HighSchoolCareerStore/View | SOURCE | 변화구 성장 target picker, 1회/최대 3회 묶음 명령, 세션별 결과와 관계·각성·중요 경기·피로·팔 상태·phase 중단 규칙; HS Core/Application/Presentation tests | 대기 |
| P-007 | 관계 이벤트 | HighSchoolCareerStore/View | SOURCE | 관계 catalog/voice/projection; `HighSchoolApplicationFlowTests`, screen projection tests | 대기 |
| P-008 | 중요 경기 | PitchSession/PitchScenario | SOURCE | 저장된 lineup/scenario/checkpoint, commit-before-3D, 다중 타자; pitch persistence tests | 대기 |
...
| P-012 | 대회/랭킹/리그 | TournamentBracket/ProspectRanking/LeagueTable | SOURCE | 순수 정렬·대회·랭킹 Core와 실제 shell rows; Core/Presentation tests | 대기 |
| P-013 | 드래프트 | forecast/resolveDraft | SOURCE | forecast/resolve state transition과 지명·미지명 분기; HS vertical tests | 대기 |
| P-014 | 회차 결산/유산 | RunRecap/LifeArchive/CareerSignatureLegacy | SOURCE | 대표 유산·기억·RunPledge 원자 정산과 archive; Core/Application tests | 대기 |
...
| P-019 | 시즌 결산 | ProCareer.reviewSeason | SOURCE | season review와 실제 결정 read model/payload; Pro tests | 대기 |
...
| P-024 | 주간 야구 노트 | WeeklyProgram | SOURCE | weekly observe/tasks/stamps/claim idempotency와 실제 rows; meta tests | 대기 |
| P-025 | 기록/리그 | RecordView/LeagueView | SOURCE | 저장 aggregate 기반 records/standings/archive projection; screen tests | 대기 |
| P-026 | 로컬 업적 | Achievements | SOURCE | 15개 한국어 업적, 개별 ack와 저장; meta/screen tests | 대기 |
| P-027 | 설정 | SettingsView | SOURCE | aggregate settings, rollback, audio focus, 알림, 접근성, reset; tests | 대기 |
| P-028 | 공유 | LifeCardView/ShareSheet | SOURCE | PNG+한국어 text, non-exported FileProvider, chooser-tapped-only EX-009; platform tests | 대기 |
...
| P-030 | 리뷰 요청 | ReviewPrompt | SOURCE | Google Play Review fail-open adapter와 조건 receipt; platform/pitch tests | 대기 |
