...
상태: SDK/런타임/정적 계약 구현 완료, Firebase·Amplitude production 수신 및 Play Data Safety 제출 증거는 미완료
...
|---|---:|---|---|
| Firebase Analytics | Unity 13.14.0 / Android 23.2.0 | 광고 ID 권한 제거, 광고 ID 수집·광고 개인화 신호 false, production config에서만 활성 | SDK/manifest/typed event 정적 검증 완료; production 수신 대기 |
| Firebase Crashlytics | Unity 13.14.0 / NDK 20.1.0 | 비식별 custom key만 허용, IL2CPP public symbols 생성·CI 업로드 | SDK/CI upload 계약 완료; test crash·symbolication 대기 |
| Amplitude Unity | v2.8 / Android 2.40.1 | AD ID, App Set ID, carrier, city, country, DMA, IP, lat/lng, region 수집 bridge option 비활성 | exact bridge key/AAR 정적 검증 완료; production 수신 대기 |
...
출시 전 GA4 Admin에서 모든 지역의 “granular location and device data collection”을 꺼 도시, 기기 모델·이름, 화면 해상도 같은 세부 자동 속성을 차단한다. 이 Console 설정의 screenshot을 릴리스 증거에 첨부한다. 제품 요구가 “대략적인 위치도 0”으로 바뀌면 Firebase Analytics를 production config에서 끄고 Amplitude만 사용하도록 빌드 계약을 변경해야 한다.
...
- [ ] GA4 granular location/device data가 모든 판매 지역에서 off이고 Google Signals/Ads 연결이 없음
...
- [ ] 개인정보처리방침 HTTPS URL과 Play Data Safety 답변이 이 표와 일치함
