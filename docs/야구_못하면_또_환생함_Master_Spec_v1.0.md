...
> v3 밸런스 기준(`PitcherPresetCatalog.swift`, `balanceVersion = 3`, 2026-07-23 동기화). 아래 수치는 실제 출시 카탈로그의 생성 직후 시작값이며, 초기 스펙의 프로토타입 수치(강속구 원석 144~147km/h·구위 65 등)를 대체한다.

| 프리셋 | 포심 시작 구속 | 구위 | 제구·커맨드 | 변화구 | 체력 |
|---|---:|---:|---:|---:|---:|
| 강속구 원석 | 141.0 km/h | 42 | 34 | 36 | 38 |
| 정교한 제구형 | 134.0 | 36 | 43 | 37 | 39 |
| 변화구 아티스트 | 136.0 | 37 | 36 | 44 | 36 |
| 체력형 선발 | 137.0 | 36 | 39 | 36 | 44 |

구위·제구·커맨드·변화구·체력은 20~80 척도의 선수 능력이다. 현재 구현은 제구와 커맨드를 하나의 커맨드 능력으로 관리하며, 세부 제구/커맨드 수치는 구종별 프로필에 있다. 총 능력 예산은 프리셋 간 비슷하게 맞추고 강점과 약점의 분포만 다르게 한다. 수치는 대량 시뮬레이션과 플레이테스트로 계속 조정한다.
