...
public struct HighSchoolCareerEngine: Sendable {
    ...
    static func highSchoolBaseline(
        ...
        // 영점은 **그 시대의 실측 RA9**다. 판정식을 실제 프로 리그 수준으로 옮기면서 리그 득점이
    ...
    static func draftEvaluationCore(state: HighSchoolCareerSnapshot) -> DraftEvaluationComponents {
        ...
        // 있었다. 판정식을 실제 프로 리그 수준(삼진 18%·피안타 24%·실점 0.12)으로 바꾸자 같은 투구가
    ...
    ///
    /// v4 보정이 +2에서 **+5**가 됐다(62/66/70). 이유는 실측이다: 판정식이 실제 리그
    /// 수준으로 옮겨 오면서 평가 분포가 당락선 바로 아래(중앙값 62)에 몰렸고, 그래서
    /// **손을 한 번도 안 맞히는 자동 진행이 20시드 중 11번 지명을 받았다**(55%). 상대 타자를
    /// 아무리 올려도 이 분포는 잘 안 갈린다 — 타자 능력 +1당 지명률 약 −1%p라, 10%에
    /// 닿으려면 고교 타자가 프로 평균보다 강해져야 해서 설정이 깨진다.
    ///
    /// 문턱을 옮기는 쪽이 정직하다. 60시드 실측(중립 릴리스·상식적 훈련):
    /// 문턱 63 → 55%, 65 → 28%, **66 → 13%**, 67 → 5%. 목표(10%)에 닿는 자리는 66이고,
    /// 67은 3년이 거의 언제나 헛수고가 된다. 여기 숫자는 **실력 0의 하한**이라, 릴리스를
    /// 맞히는 실제 사람은 이보다 확실히 높다.
    ///
    /// v3를 건드리지 않는 이유: 진행 중인 옛 저장의 당락선이 업데이트만으로 바뀌면
    /// 예고 화면이 3년 내내 약속한 숫자와 실제 지명이 어긋난다.
    ...
    private func replacing(_ state: HighSchoolCareerSnapshot, revision: UInt64? = nil,
        ...
        let nextScenario: ImportantGameScenarioContent? = currentGameScenario ?? state.currentGameScenario
