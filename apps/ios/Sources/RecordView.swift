    ...
    var body: some View {
        List {
            if let state = career.state {
                ...
                Section("주요 기록") { ForEach(Array(state.milestones.reversed()), id: \.self) { milestone in Label(milestone, systemImage: "star.circle").foregroundStyle(BaseballTheme.milestone) } }
            }
        }
