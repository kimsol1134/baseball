...
extension HighSchoolCareerStore {
    ...
    func finishTutorialPitch() {
        if countsTowardWeeklyProgram {
            CareerTelemetry.logOnce(.firstPitch)
        }
        completePrologue()
    }
    ...
    func commitTraining(
        ...
        if countsTowardWeeklyProgram {
            CareerTelemetry.log(.careerTrainingCompleted, [
    ...
    static func gainHeadline(_ gains: [AbilityGain]) -> String {
    ...
    func advanceChapter() {
        ...
        if countsTowardWeeklyProgram {
            ...
            CareerTelemetry.log(.chapterAdvanced, [
                "chapter": chapter,
    ...
    func resolveDraft() {
        ...
        if let draft = result?.snapshot.draftResult {
            if countsTowardWeeklyProgram {
                CareerTelemetry.log(.draftResolved, [
