...
extension HighSchoolCareerStore {
    ...
    func retryPendingGameCompletion(
        ...
        if !expired {
            ...
            guard weekly.record(
                .playedOnTwoDays,
                receiptID: "played-day:\(CareerPlayClock.dayKey(for: completion.completedAt))",
                now: completion.completedAt,
                calendar: calendar
            ) else { return false }
        }
        ...
        if CareerTelemetry.logOnce(
            ...
            CareerPlayClock.recordPlay(now: completion.completedAt)
        }
        CareerTelemetry.logOnce(.activationFirstGame)
