...
struct RunRecapView: View {
    ...
    private var stamps: [(text: String, tone: Color)] {
        ...
        let team = HighSchoolConclusionPresentation.localizedLifeTeamName(
            recap.record.teamName, resolver: copyResolver
        ) ?? copyResolver.resolve(.archiveTeamUnknown)
        items.append((recap.record.drafted
                      ? copyResolver.resolve(.recapStampDrafted, arguments: [.userText(team)])
                      : copyResolver.resolve(
                        .recapStampUndrafted,
                        arguments: [.integer(recap.record.evaluationScore)]
                      ),
                      recap.record.drafted ? BaseballTheme.action : BaseballTheme.textSecondary))
        items.append((copyResolver.resolve(
            .recapStampRecord,
            arguments: [.integer(recap.record.games), .integer(recap.record.strikeouts)]
        ), BaseballTheme.positive))
        if let nickname = recap.record.nicknames?.last {
            let title = HighSchoolConclusionPresentation.localizedNicknameTitle(nickname, resolver: copyResolver)
            items.append((copyResolver.resolve(.recapStampNickname, arguments: [.userText(title)]),
                          BaseballTheme.milestone))
        }
        if let rawPledgeTitle = recap.pledgeTitle {
            let pledgeTitle = recap.pledgeID.map {
                LegacyPresentation.archivedPledgeTitle(
                    id: $0, rawTitle: rawPledgeTitle, resolver: copyResolver
                )
            } ?? (copyResolver.language == .korean ? rawPledgeTitle : GameCopyResolver.unavailableText)
            let progress = recap.pledgeProgress
            let progressLine = progress.map {
                LegacyPresentation.pledgeProgress($0, resolver: copyResolver)
            } ?? GameCopyResolver.unavailableText
            let status: String
            if recap.pledgeAchieved {
                status = copyResolver.resolve(
                    .recapGoalAchieved,
                    arguments: [.userText(pledgeTitle), .integer(recap.pledgeRewardPermille / 10)]
                )
            } else if (progress?.ratioPermille ?? 0) >= 800 {
                status = copyResolver.resolve(
                    .recapGoalClose,
                    arguments: [.userText(pledgeTitle), .userText(progressLine)]
                )
            } else {
                status = copyResolver.resolve(
                    .recapGoalIncomplete,
        ...
        if let rivalLine = recap.rivalLine {
            items.append((LegacyPresentation.rivalLine(rivalLine, resolver: copyResolver),
                ...
                VStack(alignment: .leading, spacing: BaseballMetrics.stackSpacing) {
                    ...
                    if let signature = recap.record.signatureLegacy {
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(verbatim: copyResolver.resolve(.recapSignatureTitle))
                                .eyebrowStyle(BaseballTheme.milestone)
                            Text(verbatim: localizedSignature.title)
                            ...
                            Text(verbatim: copyResolver.resolve(.recapSignatureCandidates))
                                .font(.caption)
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(verbatim: copyResolver.resolve(.recapPointsTitle))
                            .eyebrowStyle(BaseballTheme.milestone)
                        Text("+\(shownSoul)")
                    ...
                    if let intent = recap.suggestedIntent,
                       let pledge = RunPledge.pledge(id: intent.pledgeID) {
                        let pledgeTitle = LegacyPresentation.pledgeTitle(pledge, resolver: copyResolver)
                        let pledgeTier = LegacyPresentation.pledgeTier(pledge.tier, resolver: copyResolver)
                        let reason = intent.reason == RunPledge.retryIntentReason
                            ? copyResolver.resolve(.recapIntentRetry)
                            : copyResolver.resolve(.recapIntentArchive)
                        VStack(alignment: .leading, spacing: 6) {
                            Text(verbatim: copyResolver.resolve(.recapIntentTitle))
                            ...
                            Button {
                                ...
                                Text(
                                    verbatim: copyResolver.resolve(
                                        intentSaved ? .recapIntentSaved : .recapIntentSave
                                    )
                                )
                            }
                            .font(.footnote.weight(.bold))
                            ...
                            .accessibilityLabel(
                                copyResolver.resolve(
                                    .recapIntentAccessibility,
                                    arguments: [
                                        .userText(pledgeTier), .userText(pledgeTitle),
