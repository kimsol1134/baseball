...
struct ArchivedPledgePresentation: Equatable {
    ...
    static func resolve(_ record: LifeRecord) -> ArchivedPledgePresentation? {
...
struct LifeArchiveSection: View {
    ...
    private var orderedRecords: [LifeRecord] {
    ...
    private var nextStrikeoutMilestone: Int? {
        CareerDisplayRules.nicknameStrikeoutLadder.first { $0 > totals.strikeouts }
    }

    /// 한 회차 최다 탈삼진 — 역대 최고 기록은 깨라고 있는 것이다.
    private var bestStrikeoutLife: LifeRecord? {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                if !records.isEmpty {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text(
                            verbatim: copyResolver.resolve(
                                collectedNicknames.count >= CareerDisplayRules.nicknameCatalogCount
                                    ? .archiveNicknameComplete : .archiveNicknameIncomplete,
                                arguments: [
                                    .integer(collectedNicknames.count), .integer(CareerDisplayRules.nicknameCatalogCount),
                                ]
                            )
                        )
                    }
...
private struct PlayerLineageRibbon: View {
    ...
    private var ordered: [LifeRecord] {
...
private struct LifeArchiveRow: View {
    ...
    init(record: LifeRecord, initiallyExpanded: Bool = false) {
        self.record = record
