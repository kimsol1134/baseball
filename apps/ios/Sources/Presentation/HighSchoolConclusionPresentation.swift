...
enum HighSchoolConclusionPresentation {
    ...
    private static func localizedAdmissionChronicle(
        ...
        for region in HighSchoolCareerStore.regions {
            if let school = HighSchoolCareerStore.schools(for: region).first(where: { $0.name == rawSchool }) {
                return resolver.resolve(
    ...
    static func localizedSchoolName(_ rawName: String?, resolver: GameCopyResolver) -> String {
        ...
        for region in HighSchoolCareerStore.regions {
            if let school = HighSchoolCareerStore.schools(for: region).first(where: { $0.name == rawName }) {
    ...
    static func localizedLifeCastName(
        ...
        guard resolver.language != .korean else { return rawName }
        for region in HighSchoolCareerStore.regions {
            for school in HighSchoolCareerStore.schools(for: region) where school.name == schoolName {
