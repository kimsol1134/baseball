...
private struct ResultBanner: View {
    ...
    var body: some View {
        ...
        .padding().background(.orange.opacity(0.1)).accessibilityElement(children: .combine)
    }
}
    ...
    var body: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 18) {
                Text("IMPORTANT MOMENT · WEEK \(state.week)").font(.caption.weight(.bold)).foregroundStyle(.orange)
                        ...
                        Button { career.selectedApproach = approach } label: {
                            HStack(spacing: 12) {
                                Image(systemName: career.selectedApproach == approach ? "checkmark.circle.fill" : "circle")
                                    .foregroundStyle(career.selectedApproach == approach ? .orange : .secondary)
                            ...
                            .background(career.selectedApproach == approach ? Color.orange.opacity(0.12) : Color.secondary.opacity(0.08), in: RoundedRectangle(cornerRadius: 12))
                        }
