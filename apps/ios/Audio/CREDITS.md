...
**심판 목소리는 거리감을 입혔다.** 원본은 성우의 근접 스튜디오 녹음이라 그대로 틀면
관중·포구의 구장 거리감 위에서 목소리만 귓속에서 난다. 120Hz 고역 통과 + 7.8kHz 저역
통과(공기 감쇠) + 44/68ms 초기 반사(백스톱 반향)를 얹었다 — 합성 시절에 반사음 레이어를
일부러 넣었던 것과 같은 이유다.

**변주(-2, -3 접미사).** 포구음은 매 투구, 타격음은 매 타구마다 나는데 같은 파일이
반복되면 몇 이닝 안에 "녹음 튼 소리"로 들린다. 탄식은 원본의 실제 다른 테이크로,
함성은 같은 폭발의 다른 트림+미세 재생 속도로, 충돌음·콜은 ±2.5~4% 리샘플(게임 오디오의
표준 피치 랜덤화를 오프라인으로)로 만들었고 SoundBank가 돌려 가며 낸다.
