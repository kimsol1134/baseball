...
function storyParagraphs(
  ...
  return [`이번 변화는 기록지보다 ${team} 내부에서 먼저 감지됐다. ${player}의 선택을 지켜본 지도자와 동료들의 반응이 이후 훈련 분위기까지 바꾸고 있다.`, `관계가 좋아졌다고 곧바로 결과가 보장되는 것은 아니다. 다음 경기에서 사인과 역할을 어떻게 실행하는지가 이번 대화의 진짜 평가가 된다.`];
}
