...
const PRIMARY = "diamond-soul.pro-career.autosave.v1";
...
export interface ProCareerAutosavePayload {
  format: "DiamondSoulProCareerAutosave";
  schemaVersion: 1;
  savedAt: string;
...
function decode(raw: string): ProCareerAutosavePayload {
  ...
  if (value.format !== "DiamondSoulProCareerAutosave" || value.schemaVersion !== 1 || !value.proCareer || !value.highSchoolCareer) throw new Error("프로 저장 형식이 유효하지 않습니다.");
