...
export function ProCareerView({ result, isRunning, error, onSign, onPlan, onPlanBlock, onGame, onReview, onOffseason, onBack }: Props) {
  ...
  return <main className="career-shell pro-career-shell">
    ...
    <div className="career-grid">
      <section className="career-panel career-player"><div className="lab-card-heading"><span>시즌 기록</span><small>{state.level === "major" ? "1군" : "2군"}</small></div>
      ...
      <section className="career-panel career-decision"><div className="lab-card-heading"><span>지금 할 일</span><small>{state.phase === "weekly_plan" ? "이번 주" : state.phase === "important_game" ? "중요 경기" : state.phase === "season_review" ? "시즌 마무리" : state.phase === "offseason_decision" ? "오프시즌" : state.phase === "contract_offer" ? "신인 계약" : "커리어"}</small></div>
        {state.phase === "contract_offer" ? <div className="career-milestone"><span>신인 계약</span><h3>지명 구단과 첫 계약을 맺습니다.</h3><p>계약 뒤 2군 선발 경쟁부터 시작하며, 고교 기록과 구종은 그대로 이어집니다.</p><button className="lab-primary" disabled={isRunning} onClick={() => void onSign()}>신인 계약 서명</button></div> : null}
        ...
        {state.phase === "important_game" ? <div className="career-milestone"><span>{state.week}주차 중요 경기</span><h3>{state.level === "major" ? "1군에서 자리를 정할 승부" : state.managerTrust < 55 ? "보직 경쟁 평가전" : "다음 보직을 결정할 경기"}</h3><p>한 점 차, 1사 2루. 이 결과는 감독 신뢰와 다음 보직에 바로 반영됩니다. 최정우부터 직접 상대합니다.</p><button className="lab-primary" disabled={isRunning} onClick={() => void onGame()}>마운드로 나가기</button></div> : null}
        {state.phase === "season_review" ? <div className="career-milestone"><span>시즌 종료</span><h3>{state.season}시즌이 끝났습니다.</h3><button className="lab-primary" disabled={isRunning} onClick={() => void onReview()}>시즌 기록 확인</button></div> : null}
        ...
        {state.phase === "retirement_decision" ? <div className="career-milestone"><span>마지막 시즌</span><h3>마지막 공을 기록에 남깁니다.</h3><button className="lab-primary" disabled={isRunning} onClick={() => void onOffseason("retire")}>은퇴식 진행</button></div> : null}
