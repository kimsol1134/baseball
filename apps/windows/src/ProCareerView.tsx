...
export function ProCareerView({ result, isRunning, error, onSign, onPlan, onPlanBlock, onGame, onReview, onOffseason, onBack }: Props) {
  ...
  return <main className="career-shell pro-career-shell stage-layout" data-stage={stage} data-team={state.team.id}>
    <section className="career-hero"><div><p className="eyebrow">프로 커리어 · {state.season}시즌</p><h2>{state.team.name} · {state.age}세</h2><p>{state.level === "major" ? "1군" : "2군"} {ROLE_LABELS[state.role]} · {state.week}/24주</p></div>
    ...
    <div className="career-grid">
      <section className="ds-card ds-player-card career-panel career-player"><div className="lab-card-heading"><span>시즌 기록</span><small>{state.level === "major" ? "1군" : "2군"}</small></div>
        <div className="ds-record-grid career-rating-grid"><div><span>경기</span><strong>{state.currentStats.games}</strong></div><div><span>선발</span><strong>{state.currentStats.starts}</strong></div><div><span>탈삼진</span><strong>{state.currentStats.strikeouts}</strong></div><div><span>ERA</span><strong>{era}</strong></div></div>
        ...
        <div className="pro-character-duo"><div><span>보직 경쟁자</span><strong>{state.team.positionCompetitor}</strong><small>{state.team.competitorRecord}</small><p>{state.team.competitorProfile}</p></div><div><span>감독</span><strong>{state.team.proCoach}</strong><small>{state.team.coachRecord}</small><p>{state.team.coachProfile}</p></div></div>
      ...
      <section className="ds-card ds-card--raised career-panel career-decision"><div className="lab-card-heading"><span>지금 할 일</span><small>{state.phase === "weekly_plan" ? "이번 주" : state.phase === "important_game" ? "중요 경기" : state.phase === "season_review" ? "시즌 마무리" : state.phase === "offseason_decision" ? "오프시즌" : state.phase === "contract_offer" ? "신인 계약" : "커리어"}</small></div>
        ...
        {state.phase === "weekly_plan" ? <><h3>{isProDebut ? "프로 데뷔를 준비합니다." : `${state.week + 1}주차`}</h3><p>{isProDebut ? "첫 공식 등판 전, 성장과 회복 중 이번 주의 우선순위를 정하세요." : `현재 피로 ${state.fatigue}, 감독 신뢰 ${state.managerTrust}. 성장, 회복, 보직 경쟁 중 이번 구간의 우선순위를 고르세요.`}</p><div className="career-training-grid">{PLANS.map((item) => <button key={item.id} className={plan === item.id ? "is-selected" : undefined} aria-pressed={plan === item.id} onClick={() => setPlan(item.id)}><strong>{item.title}</strong><span>{item.copy}</span></button>)}</div><div className="pro-plan-actions"><button className="ds-button ds-button--primary lab-primary" disabled={isRunning} onClick={() => void onPlan(plan)}>{isProDebut ? "데뷔 주간 시작" : "1주 진행"}</button><button type="button" disabled={isRunning || isProDebut} onClick={() => void onPlanBlock(plan)}>같은 계획으로 3주 진행<small>{isProDebut ? "첫 공식 등판 뒤부터 사용할 수 있습니다" : "중요 경기·보직 변화에서 자동으로 멈춤"}</small></button></div></> : null}
        {state.phase === "important_game" ? <div className="career-milestone"><span>{isMajorDebut ? "1군 데뷔전" : `${state.week}주차 중요 경기`}</span><h3>{isMajorDebut ? "처음으로 1군 마운드에 오릅니다." : state.level === "major" ? "1군에서 자리를 정할 승부" : state.managerTrust < 55 ? "보직 경쟁 평가전" : "다음 보직을 결정할 경기"}</h3><div className="rival-scouting"><strong>최정우 · 몸쪽 사냥형</strong><small>통산 508홈런 · 18시즌 연속 두 자릿수 홈런</small><p>몸쪽 공도 피하지 않고 잡아당깁니다. 불리한 카운트에서도 장타를 버리지 않습니다.</p></div><p>한 점 차, 1사 2루. 이 결과는 감독 신뢰와 다음 보직에 바로 반영됩니다.</p><button className="ds-button ds-button--primary lab-primary" disabled={isRunning} onClick={() => void onGame()}>{isMajorDebut ? "1군 데뷔 마운드로" : "마운드로 나가기"}</button></div> : null}
        ...
        {state.phase === "completed" ? <div className="draft-result is-drafted"><span>프로 기록 완료</span><h3>{state.hallOfFameScore && state.hallOfFameScore >= 70 ? "명예의 전당 헌액" : "프로 커리어 종료"}</h3><p>통산 평가 {state.hallOfFameScore} · {state.careerStats.length}시즌 · 수상 {state.awards.length}회</p></div> : null}
        {error ? <p className="error-message" role="alert">{error}</p> : null}
      </section>
      <aside className="ds-card ds-record-grid career-panel career-news"><div className="lab-card-heading"><span>구단·리그 뉴스</span><small>자동 저장됨</small></div>{state.awards.length > 0 ? <div className="award-strip"><span>수상</span>{state.awards.slice(-3).reverse().map((award) => <strong key={award}>{award}</strong>)}</div> : null}{state.news.slice(0, 9).map((item, index) => <article key={`${index}-${item}`}><span>{index === 0 ? "최신" : "이전"}</span><p>{item}</p></article>)}</aside>
    </div>
  </main>;
}
