  ...
  return <main className="career-shell pro-career-shell stage-layout" data-stage={stage} data-team={state.team.id}>
    ...
    <div className="career-grid">
      <section className="ds-card ds-player-card career-panel career-player"><div className="lab-card-heading"><span>시즌 기록</span><small>{state.level === "major" ? "1군" : "2군"}</small></div>
        ...
        <div className="pro-character-duo"><CharacterProfile label="보직 경쟁자" title={state.team.positionCompetitor} record={state.team.competitorRecord} description={state.team.competitorProfile} /><CharacterProfile label="감독" title={state.team.proCoach} record={state.team.coachRecord} description={state.team.coachProfile} /></div>
      ...
      <section className="ds-card ds-card--raised career-panel career-decision"><div className="lab-card-heading"><span>지금 할 일</span><small>{state.phase === "weekly_plan" ? "이번 주" : state.phase === "important_game" ? "중요 경기" : state.phase === "season_review" ? "시즌 마무리" : state.phase === "offseason_decision" ? "오프시즌" : state.phase === "contract_offer" ? "신인 계약" : "커리어"}</small></div>
        ...
        {state.phase === "important_game" ? <div className="career-milestone"><span>{isMajorDebut ? "1군 데뷔전" : `${state.week}주차 중요 경기`}</span><h3>{isMajorDebut ? "처음으로 1군 마운드에 오릅니다." : state.level === "major" ? "1군에서 자리를 정할 승부" : state.managerTrust < 55 ? "보직 경쟁 평가전" : "다음 보직을 결정할 경기"}</h3><CharacterProfile className="rival-scouting" title="오재민 · 몸쪽 사냥형" record="최근 5시즌 132홈런 · 장타율 .557" description="몸쪽 공도 피하지 않고 잡아당깁니다. 불리한 카운트에서도 장타를 버리지 않습니다." /><p>한 점 차, 1사 2루. 이 결과는 감독 신뢰와 다음 보직에 바로 반영됩니다.</p><button className="ds-button ds-button--primary lab-primary" disabled={isRunning} onClick={() => void onGame()}>{isMajorDebut ? "1군 데뷔 마운드로" : "마운드로 나가기"}</button></div> : null}
