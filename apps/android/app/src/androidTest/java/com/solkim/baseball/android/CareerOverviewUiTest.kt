...
class CareerOverviewUiTest {
    ...
    private fun fixture(): GameAggregateState {
        val initial = CareerFixtures.startHighSchool(HighSchoolPhase4StartRequest("918220", "power_prospect", "overview-qa", "2026-W37", "2026-09-08"))
        val run = initial.run.copy(phase = HighSchoolPhase.LEGACY,
            ...
            legacyOptions = CareerFixtures.signatureLegacyOptions().map { it.first },
        ...
        val hs = CareerFixtures.commitShadow(initial.copy(run = CareerFixtures.resignRun(run), seasonLog = (1..8).map { line.copy(chapter = it, gameNumber = it) },
            achievements = listOf(CareerFixtures.firstStrikeoutAchievementId()), unacknowledgedAchievements = listOf(CareerFixtures.firstStrikeoutAchievementId())))
        val pro = CareerFixtures.startDirectPro(ProStartDirectRequest("918220", "power_prospect", "민서준"))
        return GameAggregateState.initial("overview-qa").withCareers(stage = GameStage.HIGH_SCHOOL, highSchool = hs, pro = pro.copy(
            phase = ProCareerPhase.LEGACY_SELECTION, currentStats = pro.currentStats.copy(games = 12, inningsOuts = 90, strikeouts = 42),
            legacyCandidates = CareerFixtures.signatureLegacyOptions().map { ProLegacyCandidate(it.first, it.second, "", "", 1) }))
    }
    ...
    @Test fun recapIsCompactAndLookingAtLegacyNeverCommitsIt() {
        val state = fixture().withCareers(pro = null)
        var captured: ScreenUiAction? = null
        compose.setContent { BaseballMigrationTheme {
            CareerShell(state, false, null, ScreenId.P014_RUN_RECAP, ScreenCommandContext(), onNavigate = {}, onAction = { captured = it })
        } }
        ...
        assertEquals(ScreenProjection.project(state, ScreenId.P014_RUN_RECAP).actions.first { it.id == captured!!.actionId }.payloads, captured!!.capturedPayloads)
    }
    @Test fun singleProLegacyStillRequiresReviewInTheActualShell() {
        val base = fixture()
        val state = base.withCareers(stage = GameStage.PRO, pro = CareerAccess.pro(base)!!.copy(legacyCandidates = CareerAccess.pro(base)!!.legacyCandidates.take(1)))
        var captured: ScreenUiAction? = null
        compose.setContent { BaseballMigrationTheme {
            CareerShell(state, false, null, ScreenId.P022_PRO_LEGACY, ScreenCommandContext(), onNavigate = {}, onAction = { captured = it })
        } }
        compose.onNodeWithTag("legacy.confirm").performScrollTo().assertIsNotEnabled()
    ...
    @Test fun historyShowsFiveThenLoadsTheRemainingGames() {
        ...
        compose.setContent { BaseballMigrationTheme { Surface(color = BaseballColors.canvas) {
            Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                CompactCareerOverview(state, ScreenProjection.project(state, ScreenId.P011_HIGH_SCHOOL_CAREER), {})
            }
        } } }
    ...
    @Test fun allOverviewSurfacesRenderInEnglishAndJapaneseAtLargeText() {
        val state = fixture()
        var screen by mutableStateOf(ScreenId.P019_PRO_SEASON)
        ...
        compose.setContent {
            ...
            CompositionLocalProvider(LocalConfiguration provides config, LocalDensity provides Density(LocalDensity.current.density, 1.6f)) {
                BaseballMigrationTheme { Surface(color = BaseballColors.canvas) {
                    key(screen, tag) {
                        Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                            val model = ScreenProjection.project(state, screen).localized(GameCopy(GameLanguage.fromTag(tag)), state)
                            if (screen == ScreenId.P014_RUN_RECAP) {
        ...
        for (language in listOf("en", "ja")) for (target in compactCareerScreens + ScreenId.P014_RUN_RECAP) {
            compose.runOnIdle { tag = language; screen = target }
            compose.waitForIdle()
            assertTrue(compose.onRoot().fetchSemanticsNode().boundsInRoot.height > 0)
            if (target == ScreenId.P014_RUN_RECAP) {
                compose.onNodeWithTag("recap.hero").assertIsDisplayed()
            ...
            if (target == ScreenId.P022_PRO_LEGACY) {
            ...
            if (target == ScreenId.P026_ACHIEVEMENTS) {
                compose.onNodeWithTag("achievement.${CareerFixtures.firstStrikeoutAchievementId()}").performClick()
