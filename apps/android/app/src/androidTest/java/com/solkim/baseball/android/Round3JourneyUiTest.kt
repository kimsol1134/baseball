...
import com.solkim.baseball.application.fixtures.ProSeasonDecision
import com.solkim.baseball.application.fixtures.ProSeasonDecisionType
import com.solkim.baseball.application.fixtures.ProDecisionChoice
import com.solkim.baseball.application.fixtures.ProDecisionEffect
...
class Round3JourneyUiTest {
    ...
    private fun base(pro: ProState): GameAggregateState = GameAggregateState.initial("round3-ui").withCareers(stage = GameStage.PRO, pro = pro)

    @Test fun contractCardsOnlySelectAndSigningIsExplicit() {
        val pro = CareerFixtures.startLinkedPro(ProStartLinkedRequest("918220", "hs-ui", "QA", CareerFixtures.pitcherForPreset("power_prospect", "QA"), CareerFixtures.firstProTeamId(), 88, draftRound = 4, overallPick = 38, signingBonus = 120_000_000))
        val state = base(pro)
        val model = ScreenProjection.project(state, ScreenId.P016_PRO_CONTRACT)
        ...
        compose.setContent {
            ...
            CompositionLocalProvider(androidx.compose.ui.platform.LocalConfiguration provides config,
                ...
                BaseballMigrationTheme { CareerShell(state, false, null, ScreenId.P016_PRO_CONTRACT, ScreenCommandContext(), {}, { calls++ }) }
            }
        }
    ...
    @Test fun slumpChoiceShowsCostBeforeExplicitCommitInEveryLanguage() {
        val start = CareerFixtures.startDirectPro(ProStartDirectRequest("918220", "power_prospect", "QA"))
        ...
        val pro = start.copy(phase = ProCareerPhase.SEASON_DECISION, week = 1, seasonSegment = CareerFixtures.proSegment(1), fatigue = 50, pendingDecision = decision).let { it.copy(commitment = CareerFixtures.proCommitment(it)) }
        var language by mutableStateOf("ko")
        var calls = 0
        compose.setContent {
            ...
            CompositionLocalProvider(androidx.compose.ui.platform.LocalConfiguration provides config) { BaseballMigrationTheme {
                CareerShell(base(pro), false, null, ScreenId.P019_PRO_SEASON, ScreenCommandContext(), {}, { calls++ })
            } }
        }
    ...
    @Test fun conversationReceiptHasNavigationAndNeverReappliesTheDecision() {
        ...
        val pro = CareerFixtures.startDirectPro(ProStartDirectRequest("918220", "power_prospect", "QA"))
        val state = base(pro)
        ...
        var screen by mutableStateOf(ScreenProjection.preferredScreen(state))
        var commands = 0
        try {
            compose.setContent { BaseballMigrationTheme { CareerShell(state, false, null, screen, ScreenCommandContext(), { screen = it }, { commands++ }) } }
    ...
    @Test fun reportedDraftJourneyShowsRecordsAndDoesNotDuplicateTheStatusCard(): Unit = kotlinx.coroutines.runBlocking {
        ...
        try {
            val state = GameAggregateState.initial("draft-ui").withCareers(stage = GameStage.BETWEEN_LIVES, highSchool = CareerAccess.school(store.current))
            val record = CareerRecordPresentation.resolve(state, "hs:${CareerAccess.school(state)!!.run.careerId}")!!
            compose.setContent { BaseballMigrationTheme { CareerShell(state, false, null, ScreenId.P015_REBIRTH, ScreenCommandContext(), {}, {}) } }
            ...
            compose.onAllNodesWithText(CareerAccess.school(state)!!.run.draftResult!!.team!!.name).assertCountEquals(1)
    ...
    @Test fun actualSavedProResumeShowsSituationWithoutCommittingAnything() = kotlinx.coroutines.runBlocking {
        ...
        try {
            ...
            var calls = 0
            compose.setContent { BaseballMigrationTheme { CareerShell(state, false, null, ScreenId.P018_PRO_IMPORTANT_GAME, ScreenCommandContext(), {}, { calls++ }) } }
