...
public object ProfessionalStatusPresentation {
    ...
    public fun section(state: GameAggregateState): Phase8Section {
        ...
        val rows = when {
            pro?.phase == ProCareerPhase.COMPLETED -> listOf(Phase8Row("다음 단계", "프로 커리어 완료", pro.team.name))
            pro != null -> listOf(
                Phase8Row("다음 단계", if (pro.phase == ProCareerPhase.CONTRACT_OFFER) {
            ...
            draft?.outcome == HighSchoolDraftOutcome.DRAFTED -> listOf(
                Phase8Row("다음 단계", "지명 완료 · 입단 계약 전", "고교에서 키운 능력과 구종으로 이어갑니다."),
            ...
            draft?.outcome == HighSchoolDraftOutcome.UNDRAFTED -> listOf(
                Phase8Row("다음 단계", "이번 드래프트 미지명", "이번 생의 기록과 이어받을 힘을 남겨요."))
            else -> listOf(Phase8Row("다음 단계", "드래프트 발표 전"))
        }
        return Phase8Section("professional-status", "다음 마운드", rows)
    }
}
