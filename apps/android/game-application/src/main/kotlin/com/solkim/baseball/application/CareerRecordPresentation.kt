...
public data class CareerGameView(val id: String, val label: String, val outs: Int, val strikeouts: Int, val runs: Int,
    val walks: Int, val hits: Int, val perfect: Int, val team: Int, val opponent: Int, val manual: Boolean, val decision: String = "", val started: Boolean = false, val homeRuns: Int? = null, val pitches: Int? = null, val earnedRuns: Int? = null, val completeGame: Boolean? = null) {
    public val isShutout: Boolean get() = completeGame == true && runs == 0 && opponent == 0 && team > 0
    public val achievementTitle: String? get() = if (isShutout) "완봉승" else if (completeGame == true) "완투" else null
            ...
            val liveRows = pro.currentGameLines.filter { season == null || it.season == season }
                .distinctBy { "${it.season}:${it.week}:${it.outingNumber}" }.asReversed().map {
                    CareerGameView("pro:${pro.careerId}:${it.season}:${it.week}:${it.outingNumber}", "${it.season}시즌 · ${it.week}주차 · ${if (it.played) "직접" else "자동"}",
                        it.outs, it.strikeouts, it.runsAllowed, it.walks, it.hits, it.perfectReleases, it.teamRuns, it.opponentRuns, it.played, it.decision.wire, it.started, it.homeRuns, it.pitches, it.earnedRuns, it.completeGame)
                }
