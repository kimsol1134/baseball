...
public fun Phase8ScreenModel.localized(copy: GameCopy, state: GameAggregateState): Phase8ScreenModel {
    ...
    return this.copy(title = text(title), subtitle = text(subtitle),
        ...
        actions = actions.map { it.copy(label = copy.legacy(it.label), description = if (it.id.startsWith("selectLegacy:")) SignatureLegacyDisplay.effect(it.id.removePrefix("selectLegacy:"), copy) ?: text(it.description) else text(it.description)) },
    )
}
