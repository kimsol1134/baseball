...
public object PlayerAlbum {
    public fun validate(pages: List<PlayerAlbumPage>) {
        ...
        pages.forEach { p ->
            ...
            require(p.details.isEmpty() || p.details.size in setOf(8, 9, 11) && p.details.all { it >= 0 })
            ...
            require(p.details.size < 11 || p.details[10] <= p.details[9] && p.details[9] <= p.games)
    ...
    public fun capture(before: GameAggregateState, after: GameAggregateState): List<PlayerAlbumPage> {
        ...
        for (state in listOf(before, after)) {
            for (scope in CareerRecordPresentation.scopes(state)) {
                ...
                ).map { it.coerceAtLeast(0) } + (if (proStats.earnedRuns != null && (pending == null || pending.earnedRuns != null)) listOf((proStats.earnedRuns ?: 0) - (pending?.earnedRuns ?: 0)) + (if (proStats.completeGames != null && proStats.shutouts != null) listOf(proStats.completeGames!! - if (pending?.completeGame == true) 1 else 0, proStats.shutouts!! - if (pending?.completeGame == true && pending.runsAllowed == 0 && pending.opponentRuns == 0 && pending.teamRuns > 0) 1 else 0) else emptyList()) else emptyList()) else emptyList()
...
internal object PlayerAlbumCodec {
    ...
    fun encode(pages: List<PlayerAlbumPage>): JsonValue = a(pages.map { p ->
        val rows = p.rows.map { r -> a(buildList {
            ...
            if (r.decision.isNotEmpty() || r.started || r.homeRuns != null || r.pitches != null || r.earnedRuns != null || r.completeGame != null) addAll(listOf(s(r.decision), JsonValue.Bool(r.started)))
            if (r.homeRuns != null || r.pitches != null || r.earnedRuns != null || r.completeGame != null) addAll(listOf(r.homeRuns?.let(::n) ?: JsonValue.Null, r.pitches?.let(::n) ?: JsonValue.Null))
            if (r.earnedRuns != null || r.completeGame != null) add(r.earnedRuns?.let(::n) ?: JsonValue.Null)
            if (r.completeGame != null) add(JsonValue.Bool(r.completeGame))
        }) }
    ...
    fun decode(value: JsonValue?): List<PlayerAlbumPage> {
        ...
        return value.items().map { item ->
            ...
            PlayerAlbumPage(RecordScope(p[0].text(), p[1].text(), p[2].text()), p[3].int(), p[4].int(), p[5].int(), p[6].int(), p[7].bool(),
                p[8].items().map { itemRow -> val r = itemRow.items(); require(r.size in setOf(11, 13, 15, 16, 17))
                    CareerGameView(r[0].text(), r[1].text(), r[2].int(), r[3].int(), r[4].int(), r[5].int(), r[6].int(), r[7].int(), r[8].int(), r[9].int(), r[10].bool(), if (r.size >= 13) r[11].text() else "", if (r.size >= 13) r[12].bool() else false, if (r.size >= 15 && r[13] != JsonValue.Null) r[13].int() else null, if (r.size >= 15 && r[14] != JsonValue.Null) r[14].int() else null, if (r.size >= 16 && r[15] != JsonValue.Null) r[15].int() else null, if (r.size == 17) r[16].bool() else null) },
