...
class AnalyticsProjectorTest {
    ...
    fun everyNonZeroMatrixEventHasAContractSourceAndRetiredEventsHaveNoSource() {
        ...
        assertEquals(emptySet(), AnalyticsContract.retiredEvents intersect AnalyticsContract.semanticSources.keys)
        assertEquals(emptySet(), AnalyticsContract.intentionalZeroCallerEvents intersect AnalyticsContract.semanticSources.keys)
        assertEquals(expectedNonZeroMatrixEvents, AnalyticsContract.semanticSources.keys)
        assertTrue(AnalyticsContract.semanticSources.keys.all { it in AnalyticsContract.allowedProperties })
        assertTrue(AnalyticsContract.semanticSources.values.all(String::isNotBlank))
        assertEquals(setOf("life_card_shared", "life_card_share_completed", "daily_inning_opened", "daily_inning_rewarded"), AnalyticsContract.intentionalZeroCallerEvents)
    }

    @Test
    fun typedBoundaryCoversEveryFrozenPropertyAndKeepsTextIdsText() {
        assertEquals(AnalyticsContract.semanticSources.keys, AnalyticsContract.allowedProperties.keys)
        AnalyticsContract.propertyKinds.forEach { (eventName, kinds) ->
            assertTrue(eventName in AnalyticsContract.allowedProperties)
            assertTrue(kinds.keys.all { it in AnalyticsContract.allowedProperties.getValue(eventName) })
        }
        AnalyticsContract.allowedTextValues.forEach { (eventName, domains) ->
            domains.forEach { (key, values) ->
                assertTrue(key in AnalyticsContract.allowedProperties.getValue(eventName))
                assertTrue(key !in AnalyticsContract.propertyKinds[eventName].orEmpty())
                assertTrue(values.isNotEmpty())
            }
        }
        AnalyticsContract.validateManual(
            "run_pledge_selected",
        ...
        AnalyticsContract.allowedTextValues.forEach { (eventName, domains) ->
            domains.forEach { (key, values) ->
                AnalyticsContract.validateManual(eventName, listOf(key to values.first()))
                assertFailsWith<IllegalArgumentException> {
                    AnalyticsContract.validateManual(eventName, listOf(key to "__unknown_enum__"))
                }
            }
        }
        AnalyticsContract.propertyKinds.forEach { (eventName, kinds) ->
            kinds.forEach { (key, kind) ->
                val valid = when (kind) {
                    AnalyticsPropertyKind.TEXT -> "text"
                    AnalyticsPropertyKind.FLAG -> "true"
                    AnalyticsPropertyKind.WHOLE -> "1"
                    AnalyticsPropertyKind.DECIMAL -> "1.0"
                }
                AnalyticsContract.validateManual(eventName, listOf(key to valid))
            }
        }
        assertFailsWith<IllegalArgumentException> {
            AnalyticsProjector.toReceipt(
                ProjectedAnalyticsEvent(
                    eventName = "reminder_changed",
                    scope = "typed-boundary",
                    properties = mapOf(
                        "enabled" to AnalyticsValue.Flag(true),
                        "source" to AnalyticsValue.Text("permission_result"),
                    ),
                ),
                GameAggregateState.initial("analytics-typed-boundary"),
            )
        }
    }
    ...
    fun firstPitchIsOnlyTheNonChallengeTutorialCompletionAndNotGenericPitchCompletion() {
        val state = GameAggregateState.initial("analytics-first-pitch")
        val completion = GameCommandEnvelope(
            commandId = "generic-complete-pitch",
            sessionId = "analytics-first-pitch-session",
            expectedRevision = 0UL,
            command = GameCommand.CompletePitch("session"),
        )
        assertTrue(AnalyticsProjector.project(state, state, completion).none { it.eventName == "first_pitch" })
    }
    ...
    fun proLegacyRecordedOmitsSoulBonusWhenNoAuthoritativeSoulEvidenceExists() {
        ...
        val linked = kernel.startLinked(
            ProStartLinkedRequest(
                seed = "918220",
                highSchoolCareerId = "hs-analytics-pro",
                identityName = "프로 투수",
                pitcher = pitcher,
        ...
        val before = GameAggregateState.initial("analytics-pro").copy(
        ...
        val reduced = GameStateReducer.dispatch(
            ...
            GameCommandEnvelope(
                ...
                sessionId = "analytics-pro-session",
                expectedRevision = 0UL,
    ...
    fun trainingBlockProjectsOneReceiptPerCommittedEvidenceInOrderAndSurvivesRestart() {
        ...
        var training = kernel.start(
            HighSchoolPhase4StartRequest("918220", "power_prospect", "analytics-training", "2026-W33", "2026-08-14"),
        ).state
        ...
        val before = GameAggregateState.initial("analytics-training").copy(
        ...
        val envelope = GameCommandEnvelope(
            ...
            sessionId = "analytics-training-session",
            expectedRevision = 0UL,
    ...
    fun trainingBlockSaveFailureLeavesEvidenceAndAnalyticsUncommitted() = runBlocking {
        val kernel = HighSchoolPhase4Kernel()
        var training = kernel.start(
            HighSchoolPhase4StartRequest("918221", "power_prospect", "analytics-training-save-failure", "2026-W33", "2026-08-14"),
        ).state
        ...
        training = kernel.commitShadowState(training.copy(revision = training.run.revision))
        val before = GameAggregateState.initial("analytics-training-save-failure").copy(
            stage = GameStage.HIGH_SCHOOL,
            highSchool = training,
            meta = GameMetaState(activeHighSchoolCareerId = training.run.careerId),
        ).committed()
        ...
        val command = GameCommandEnvelope(
            ...
            sessionId = "analytics-training-save-failure-session",
    ...
    fun manualMatrixInteractionRejectsInventedPropertyAndRetiredOrZeroCaller() {
        val state = GameAggregateState.initial("analytics-matrix-contract")
    ...
    fun aggregateTransitionProjectionUsesOnlyKnownMatrixProperties() = runBlocking {
        val initial = GameAggregateState.initial("analytics-projector")
        ...
        assertTrue(AnalyticsContract.nonRetiredEvents.contains("onboarding_started"))
        val enter = GameCommandEnvelope("enter-setup", "analytics-enter", 0UL, GameCommand.EnterSetup)
        ...
        val payload = ScreenPayloads.analytics(
            state,
        ...
        assertTrue(receipt.properties.map { it.first }.all { it in AnalyticsContract.allowedProperties.getValue("reminder_changed") })
        assertTrue(result.state.analytics.receipts.none { it.eventName in AnalyticsContract.intentionalZeroCallerEvents })
    }

    @Test
    fun durableNativeHandoffFailureCannotReplayHistoricReceiptAfterRestart() = runBlocking {
        val initial = GameAggregateState.initial("analytics-handoff")
        val repository = InMemoryShadowFixtureGameStoreRepository(initial)
        ...
        val restarted = KotlinGameStore.open("analytics-handoff", repository, NativeAuthorityMode.NATIVE_SHADOW_READ_ONLY, restartedProjection)
    ...
    fun observerFailureAfterSaveIsRetryableAndDoesNotHideCommittedReceipt() = runBlocking {
        val initial = GameAggregateState.initial("analytics-observer-retry")
        val repository = InMemoryShadowFixtureGameStoreRepository(initial)
        var fail = true
        val published = mutableListOf<String>()
        val projection = AnalyticsReceiptProjection(
    ...
    fun durableLedgerObserverFailureCannotMakeDispatchFail() = runBlocking {
        val initial = GameAggregateState.initial("analytics-ledger-failure")
        val repository = InMemoryShadowFixtureGameStoreRepository(initial)
        val published = mutableListOf<String>()
        val projection = AnalyticsReceiptProjection(
    ...
    private class FailingStoreRepository : GameStoreRepository {
        override suspend fun save(value: GameAggregateState, revision: ULong): SaveWriteResult<GameAggregateState> =
            throw SaveRepositoryException(SaveFailureCode.IO_FAILED, "analytics.training_save_failure")
