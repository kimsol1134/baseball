...
namespace Baseball.Presentation.Setup
{
    public sealed class SetupScreenController : BaseballScreenControllerBase
    {
        ...
        protected override void AddCustomContent(VisualElement host, BaseballScreenViewModel viewModel, IShellNavigator navigator)
        {
            ...
            {
                ChoiceCard card = null;
                card = new ChoiceCard(option.Title, option.Detail + " " + option.EffectSummary, "screen-setup-preset-" + option.Id, () =>
                {
            ...
            AddSingleChoice(host, navigator, "난이도", "한 가지를 선택합니다.", "difficulty",
                options.Difficulties, advanced.SetupDifficulty, advanced.SetSetupSingle);
            ...
            AddSingleChoice(host, navigator, "자동 계승 야구혼 성향", "자동 계승 수치를 어느 성장축에 먼저 반영할지 고릅니다.", "soul_domain",
                WithNone(options.SoulDomains, "자동 배분"), advanced.SetupSoulDomain, advanced.SetSetupSingle);
            ...
            AddMultiChoice(host, navigator, "상속 기억", "최대 4개를 이번 인생에 가져옵니다.", "memory",
                memories, advanced.IsSetupSelected, advanced.ToggleSetupMulti);
            AddSingleChoice(host, navigator, "대표 유산", "해금한 대표 유산 중 하나를 장착합니다.", "signature",
                WithNone(options.SignatureLegacies, "장착하지 않음"), advanced.SetupSignatureLegacy, advanced.SetSetupSingle);
        }
        ...
        private static IReadOnlyList<CareerChoiceReadModel> WithNone(
            IReadOnlyList<CareerChoiceReadModel> values,
            string noneTitle)
        {
            return new[] { new CareerChoiceReadModel("none", noneTitle, payload: string.Empty) }
                .Concat(values ?? System.Array.Empty<CareerChoiceReadModel>())
                .ToArray();
        }
