...
namespace Baseball.Core.HighSchool
{
    ...
    public sealed class CareerGameGrowth
    {
        ...
        public static CareerGameGrowth Evaluate(HighSchoolCareerSnapshot state,ImportantInningReport report)
        {
            ...
            var applied=TalentRules.Apply(state.Talent,ability,Rating(ability,state.Pitcher),1);var growthPoints=state.Talent.Grade(ability).Ceiling()>=80?1:applied.Allowed;var limit=growthPoints==0&&!applied.Bloomed.HasValue?" 재능 한계에 닿아 능력치는 오르지 않았지만 압박이 남았습니다.":"";var bloom=applied.Bloomed.HasValue?" "+AbilityLabel(applied.Bloomed.Value)+" 재능이 "+applied.Talent.Grade(applied.Bloomed.Value)+"로 만개했습니다.":"";var title=applied.Allowed>0?"경기 기반 성장 · "+AbilityLabel(ability)+" +"+applied.Allowed:growthPoints>0?"경기 기반 숙련 · "+MasteryEffectRules.DisplayName(ability):"경기 기반 성장 · "+AbilityLabel(ability)+" 한계 압박";return new CareerGameGrowth(ability,growthPoints,reason,title,evidence+bloom+limit,applied.Talent,applied.Bloomed);
        }
        public PitcherSnapshot Applying(PitcherSnapshot p){if(Points<=0)return p;var focus=Ability==TalentAbility.Stuff?TrainingFocus.Velocity:Ability==TalentAbility.Command?TrainingFocus.Command:Ability==TalentAbility.Movement?TrainingFocus.BreakingBall:TrainingFocus.Stamina;return PitcherGrowthRules.Advance(p,focus,Points).Pitcher;}
