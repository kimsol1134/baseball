...
description: >
  ...
  Apple SD Gothic Neo in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
typography:
  # — reading ramp —
  body:    { fontFamily: "GMarketSans", cqw: 1.2, weight: 500, lineHeight: 1.6 }
  lead:    { fontFamily: "GMarketSans", cqw: 1.6, weight: 500, lineHeight: 1.5 }
  caption: { fontFamily: "GMarketSans", cqw: 0.9, weight: 500, lineHeight: 1.5 }
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  # — display / hero ramp (Apple SD Gothic Neo, lowercase, negative tracking) —
  h3:      { fontFamily: "GMarketSans", cqw: 2.8, weight: 700, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "GMarketSans", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "YeogiOttaeJalnan", cqw: 4.5, weight: 400, lineHeight: 1.1, tracking: "-0.01em", lower: true }
  stat-value:{ fontFamily: "GMarketSans", cqw: 5.5, weight: 700, lineHeight: 1.0, tracking: "-0.03em" }
  h1:      { fontFamily: "YeogiOttaeJalnan", cqw: 7.5, weight: 400, lineHeight: 0.96, tracking: "-0.01em", lower: true }
  fadelist-item:{ fontFamily: "YeogiOttaeJalnan", cqw: 7.5, weight: 400, lineHeight: 1.0, tracking: "-0.01em", lower: true }
  quote-mark:{ fontFamily: "YeogiOttaeJalnan", cqw: 10.0, weight: 400, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "YeogiOttaeJalnan", cqw: 10.5, weight: 400, lineHeight: 0.96, tracking: "-0.01em", lower: true }
  display: { fontFamily: "YeogiOttaeJalnan", cqw: 13.0, weight: 400, lineHeight: 0.94, tracking: "-0.01em", lower: true }
...
components:
  ...
  footage-overlay:
    typography: "YeogiOttaeJalnan display + GMarketSans support"
    depth: "Live gameplay overlays may use a bounded dark text shadow and palette-tinted radial veil for compression-safe readability; authored flat cards remain shadowless."
    description: "App Store preview exception for type placed over moving gameplay footage."
---
...
- **Fonts** — App Store preview display is **YeogiOttaeJalnan** and supporting copy is **GMarketSans**; IBM Plex Mono remains chrome-only.
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean the remapped brand values.
- **Live footage depth** — flat authored planes remain the default; only text over moving gameplay may use the bounded shadow and radial veil declared in `components.footage-overlay`.


## Overview

Broadside at frame scale is a **protest-poster system where type is so large it stops reading as
text and becomes graphic primitive.** Apple SD Gothic Neo `display` at 13cqw puts a single lowercase word
...
**Apple SD Gothic Neo** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Apple SD Gothic Neo 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Apple SD Gothic Neo body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Apple SD Gothic Neo `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Apple SD Gothic Neo display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Apple SD Gothic Neo `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Apple SD Gothic Neo lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Apple SD Gothic Neo `display`/`h1`
...
row of three top-border-only stat-cards — big Apple SD Gothic Neo-900 numeral in `{colors.fire-orange}`, Apple SD Gothic Neo
...
**Ground** ink-black. **Composes** fadelist (3 stacked Apple SD Gothic Neo-900 words at 1.0/0.5/0.22), fadelist-title.
...
Apple SD Gothic Neo `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Apple SD Gothic Neo display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Apple SD Gothic Neo display; never add a second accent color.
...
- No serif companion; chrome is never Apple SD Gothic Neo.
...
- **Type** — Apple SD Gothic Neo lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Apple SD Gothic Neo + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
