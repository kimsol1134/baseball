...
typography:
  ...
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 600, tracking: "0.14em", upper: true }
...
- **Fonts** — already set to **Pretendard** (display) / **Arial Black** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 600, 700, 800, 900, 950}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
- **Pretendard + IBM Plex Mono are staged locally in `assets/fonts/`**; use only those project-shipped files so Korean and mono chrome render deterministically without network font loading.
